...
void demo() {
  ...
  printf("Добавить два элемента в начало дека и третий в конец- Элементы в "
         "следующем порядке : второй, первый, третий;\n");
  ...
  printf("Выбрать и удалить элемент из начала дека - вернется второй элемент, "
         "остались: первый, третий;\n");
  pop_front(d);
  print_dek(d, size);

  printf("Выбрать и удалить элемент из конца дека - вернется третий элемент, "
         "остался только первый элемент;\n");
  pop_back(d);
  print_dek(d, size);

  printf("Выбрать и удалить элемент из конца дека - вернется первый элемент, "
         "дек пуст;\n");
  pop_back(d);
  print_dek(d, size);

  printf("ыбрать и удалить элемент из начала дека - Программа продолжает "
         "работать, но возвращается код неудачного выполнения операции\n");
  pop_front(d);
  print_dek(d, size);
}
